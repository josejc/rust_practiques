...
impl LCG {
    ...
    fn randst(&mut self, zset: u32, stream: usize) {
        self.x[stream] = zset;
    }

    // Return the integer actual of sequence in this stream
    fn randgt(self, stream: usize) -> u32 {
        self.x[stream]
    }

    fn rand(&mut self, stream: usize) -> f32 {
        //x = ((a * x) + c ) % m; Panic multiply with overflow
        let mut aux: u64;       // Solution to overflow of u32 
    
        aux = self.a as u64 * self.x[stream] as u64;
        aux = aux + self.c as u64;
        aux = aux % self.m as u64;
        self.x[stream] = aux as u32;
        (self.x[stream] as f32 / self.m as f32)
    }
}
...
fn main() {
    ...
    let mut r = LCG::new(630_360_016, 0, 2_147_483_647);
